    ...
    *"ART reconstrutions usually suffer from salt and pepper noise,
    ...
    def GetLambdaLines(angle_k):
        if angle_k == 0:
            ...
            line1 = lambda x2, y2, xpi1, ypi1, xpi2, ypi2, angle_k: (
                x2 - .5, ypi1 * np.ones(y2.shape))
            line2 = lambda x2, y2, xpi1, ypi1, xpi2, ypi2, angle_k: (
    ...
    for k in np.arange(iterations):
        ...
        for l in np.arange(A):
            ...
            for i in np.arange(N):
                ...
                P1x = np.uint64(
                    np.ceil(px) * (np.ceil(px) < N - 1) + (N - 1) * (np.ceil(px) >= N - 1))
                P1y = np.uint64(
                    np.ceil(py) * (np.ceil(py) < N - 1) + (N - 1) * (np.ceil(py) >= N - 1))
